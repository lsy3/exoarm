...
//                     q.w*q.w - q.x*q.x + q.y*q.y - q.z*q.z,
//                     res);
const void Quaternion::to_euler_angles(float out[]) {
  out[0]   = atan2(2.0f * (b * c + a * d), a * a - b * b + c * c - d * d);
  out[1]   = asin(-2.0f * (c * d - a * b));
  out[2]   = atan2(2.0f * (b * d + a * c), a * a - b * b - c * c + d * d);

  for(int i=0; i<3; i++) out[i] *= 180.0f / PI;
}

const void Quaternion::to_array(float out[]) {
