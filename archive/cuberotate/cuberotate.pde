...
void draw()
{
  ...
  translate(50, 0, 0);
  c1 = cos(radians(roll2));
  s1 = sin(radians(roll2));
  c2 = cos(radians(pitch2)); // intrinsic rotation
  s2 = sin(radians(pitch2));
  c3 = cos(radians(yaw2));
  s3 = sin(radians(yaw2));
  applyMatrix( c2*c3, s1*s3+c1*c3*s2, c3*s1*s2-c1*s3, 0,
               -s2, c1*c2, c2*s1, 0,
...
void serialEvent(Serial p) 
{
  ...
  if ((incoming.length() > 8))
  {
    ...
    if ( (list.length > 0) && (list[0].equals("Orientation:")) ) 
    {
      ...
      roll2  = float(list[6]); // Roll = Z
      pitch2 = float(list[5]); // Pitch = Y 
      yaw2   = float(list[4]); // Yaw/Heading = X
    }
